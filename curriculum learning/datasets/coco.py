...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target):
        ...
        if target["boxes"].shape[0] > 300:
            fields = ["boxes", "labels", "area", "iscrowd"] #add points in others when training 
            for field in fields:
...
def build(image_set, args):
    ...
    mode = 'instances'
    if args.dataset_file == "dvd":
        PATHS = {
            "train": (root / args.data_dir_label_train, root / args.label_label_root / args.annotation_json_train_label),
            "val": (root /  args.data_dir_label_val, root / args.label_label_root / args.annotation_json_val),
        }
    else:
        PATHS = {
    ...
    if args.pixels == 600:
        # The problem is here - boxes is empty
...
def build_semi_unlabel(image_set, args):
    #root = Path(args.data_path)
    root = Path(args.unlabel_label_root)
    assert root.exists(), f'provided COCO path {root} does not exist'

    mode = 'instances'
    if args.dataset_file == "dvd":
        PATHS = {
            "train": (root / args.data_dir_unlabel_train, root/ args.annotation_json_train_unlabel),
            "val": (root /  args.data_dir_label_val, root / args.label_label_root / args.annotation_json_val),
        }
    else:
        PATHS = {
