...
import numpy as np  # Import NumPy for tensor to array conversion
...
def unified_filter_pseudo_labels(pseudo_unsup_outputs, targets_unlabel_q, targets_unlabel_k, records_unlabel_q, records_unlabel_k, pixels, label_type, w_p=0.5, w_t=0.5, is_binary=False, threshold=0.7):
    ...
    #  # Convert pseudo_annotations (which may contain PyTorch tensors) to Python lists
    # pseudo_annotations_serializable = []
    # for i in range(len(pseudo_annotations)):
    #     pseudo_dict = pseudo_annotations[i]
    #     pseudo_dict_serializable = {}
    #     for key, value in pseudo_dict.items():
    #         if isinstance(value, np.ndarray):
    #             pseudo_dict_serializable[key] = value.tolist()  # Convert NumPy array to list
    #         elif isinstance(value, torch.Tensor):
    #             if value.numel() == 1:  # Check if the tensor has only one element
    #                 pseudo_dict_serializable[key] = value.item()  # Convert to scalar
    #             else:
    #                 pseudo_dict_serializable[key] = value.detach().cpu().numpy().tolist()  # Convert to NumPy array and then to list
    #         else:
    #             pseudo_dict_serializable[key] = value
    #     pseudo_annotations_serializable.append(pseudo_dict_serializable)

    # # Serialize pseudo_annotations to JSON
    # pseudo_annotations_json = json.dumps(pseudo_annotations_serializable, indent=4)

    # # Define the file path where you want to save the JSON file
    # json_file_path = 'pseudo_annotations.json'

    # # Write the JSON data to the file
    # with open(json_file_path, 'w') as json_file:
    #     json_file.write(pseudo_annotations_json)
