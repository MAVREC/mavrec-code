...
def main(args):
    ...
    if args.frozen_weights is not None:
        ...
        if 'model_student' not in checkpoint.keys():
            model_without_ddp_student.load_state_dict(checkpoint['model'])
        else:
            model_without_ddp_student.load_state_dict(checkpoint['model_student'])
            model_without_ddp_teacher.load_state_dict(checkpoint['model_teacher'])
    ...
    if args.eval:
        test_stats, coco_evaluator = evaluate(model_teacher, criterion, postprocessors,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch < args.BURN_IN_STEP:
            ...
            if args.output_dir:
                checkpoint_paths = [output_dir / 'checkpoint.pth']
                # extra checkpoint before LR drop and every save_freq epochs
                if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % args.save_freq == 0 or (epoch + 1) == args.epoch:
                    checkpoint_paths.append(output_dir / f'checkpoint{epoch:04}.pth')
                for checkpoint_path in checkpoint_paths:
                    print('-------saving model--------')
