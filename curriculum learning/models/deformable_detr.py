...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        #breakpoint()
        return out #dict_keys(['pred_logits', 'pred_boxes', 'aux_outputs']) ##out['pred_boxes'].shape torch.Size([1, 900, 4])
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # indices, (([70, 254], [0,1]), ([79,98,190,275],[2,3,0,1]))
...
class SetCriterion_semi(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, indicators):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           ...
           The target boxes are expected in format (center_x, center_y, h, w), normalized by the image size.
        """
        # different from Unbiased Teacher, we always use pseudo box for regression because we found its performance is better
        ...
        if len(batch_idx) == 0:
            ...
            indicator_weights = torch.clone(batch_idx)
            for i in range(max(batch_idx) + 1):
                indicator_weights[batch_idx == i] = indicators[i]
            indicator_weights = torch.unsqueeze(indicator_weights, 1).cuda()
            ...
            loss_giou = 1 - torch.diag(box_ops.generalized_box_iou(
                box_ops.box_cxcywh_to_xyxy(src_boxes),
                box_ops.box_cxcywh_to_xyxy(target_boxes)))
            indicator_weights = torch.squeeze(indicator_weights)
            loss_giou = loss_giou * indicator_weights
            losses['loss_giou'] = loss_giou.sum() / num_boxes
        return losses
...
def build_semi(args):
    num_classes = 20 if args.dataset_file != 'coco_semi' else 20 #default hardcoded 20 from before.
    if args.dataset_file == "coco_panoptic":
        num_classes = 250
