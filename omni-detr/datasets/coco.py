...
class CocoDetection(TvCocoDetection):
    def __init__(self, img_folder, image_set, ann_file, transforms, return_masks, cache_mode=False, local_rank=0, local_size=1):
...
class CocoDetection_semi(TvCocoDetection_semi):
    def __init__(self, img_folder, ann_file, image_set, transforms_strong, transforms_weak, return_masks, cache_mode=False, local_rank=0,
                 local_size=1):
        super(CocoDetection_semi, self).__init__(img_folder, ann_file, image_set,
                                            cache_mode=cache_mode, local_rank=local_rank, local_size=local_size)
...
def build(image_set, args):
    ...
    if args.pixels == 600:
        dataset = CocoDetection(img_folder, ann_file, image_set, transforms=make_coco_transforms600(image_set), return_masks=args.masks,
...
def build_semi_label(image_set, args):
    ...
    img_folder, ann_file = PATHS[image_set]
    if args.pixels == 600:
        dataset = CocoDetection_semi(img_folder, ann_file, image_set,
        ...
        dataset = CocoDetection_semi(img_folder, ann_file, image_set, transforms_strong=make_coco_strong_transforms_with_record(image_set), transforms_weak=make_coco_weak_transforms_with_record(image_set), return_masks=args.masks,
                                cache_mode=args.cache_mode, local_rank=get_local_rank(), local_size=get_local_size())
